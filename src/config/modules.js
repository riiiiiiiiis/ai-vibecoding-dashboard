...
export function getTermsByModule(moduleKey) {
  if (moduleKey === "basics") {
    return [
      // Новые термины
      { title: "Вайбкодинг", desc: "Подход к созданию прототипов: короткие итерации, проверка ценности, минимум декора — максимум пользы." },
      { title: "Искусственный Интеллект (AI)", desc: "Общий термин для систем, выполняющих интеллектуальные задачи: анализ, принятие решений, генерация." },
      { title: "Промптинг", desc: "Практика формулирования промптов — чётких текстовых инструкций с целями, контекстом и форматом ответа." },
      { title: "Генеративный AI", desc: "Класс моделей, способных создавать новые тексты, изображения, аудио или код на основе обученных представлений." },
      { title: "Приложение", desc: "Программный продукт, решающий задачу пользователя (веб/мобайл/десктоп)." },
      { title: "Пользовательский интерфейс (UI)", desc: "Визуальная часть приложения: элементы, раскладки, состояния, взаимодействия." },
      { title: "Модель AI", desc: "Алгоритмическая система (например, LLM), обученная на данных для инференса/генерации по входному запросу." },
      // Ранее добавленные термины
      { title: "Промпт", desc: "Текстовая инструкция/запрос для модели: цель, контекст, формат ответа, ограничения." },
      { title: "Генерация кода", desc: "Использование ИИ для синтеза исходного кода по описанию задачи." },
      { title: "Веб‑приложение", desc: "Приложение, работающее в браузере и доступное по URL." },
      { title: "No‑code (Без кода)", desc: "Подход/инструменты для сборки приложений без ручного программирования." },
      { title: "API (Application Programming Interface)", desc: "Программный интерфейс для интеграции и обмена данными между системами." },
    ];
  }
  if (moduleKey === "prompts") {
    return [
      { title: "Системный промпт", desc: "Правила и роль модели, задают общий стиль и ограничения ответов." },
      { title: "Пользовательский промпт", desc: "Конкретная задача пользователя, передаваемая модели." },
      { title: "Few‑shot", desc: "Подача нескольких примеров в промпте для уточнения формата и качества ответа." },
      { title: "Формат ответа", desc: "Ожидаемая структура: JSON/Markdown/таблица и др." },
    ];
  }
  // knowledge
  return [
    { title: "Источник", desc: "Документ или раздел, содержащий исходную информацию (PDF/MD/текст)." },
    { title: "Цитата", desc: "Фрагмент текста из источника, подтверждающий ответ модели." },
    { title: "Индекс", desc: "Структура данных для быстрого поиска релевантных фрагментов." },
    { title: "Релевантность", desc: "Степень соответствия найденного фрагмента запросу." },
  ];
}
